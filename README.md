...
claude plugin install vkskill
```

Then type `/vkskill` to run first-time setup (installs NotebookLM,
configures second model). After setup, just say "搜证", "脑暴", or "质证"
in any conversation.

The old `/zuoshi-kaopu` entry still works as a legacy alias.
...
Then restart Codex and type `vkskill` to run first-time setup.
